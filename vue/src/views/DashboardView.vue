  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <template v-if="loading">
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="i in 4" :key="i" class="bg-white rounded-xl p-5 animate-pulse">
    ...
    <template v-else-if="stats">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white shadow-sm rounded-xl p-5">
          <p class="text-xs text-gray-500 uppercase tracking-wide font-medium">Citas hoy</p>
        ...
        <div class="bg-white shadow-sm rounded-xl p-5">
          <p class="text-xs text-gray-500 uppercase tracking-wide font-medium">Ingresos mes</p>
          <p class="text-3xl font-bold text-teal-600 mt-1">{{ stats.ingresos_este_mes.toFixed(2) }}€</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white shadow-sm rounded-xl overflow-hidden">
          <div class="px-5 py-3 border-b bg-gray-50 flex justify-between items-center">
            <h2 class="font-semibold text-gray-700 text-sm">Citas de hoy</h2>
          ...
          <ul v-else class="divide-y divide-gray-100">
            <li v-for="cita in stats.proximas_citas" :key="cita.id" class="px-5 py-3 flex items-center gap-3">
              <span class="text-sm font-mono text-gray-500 w-12">{{ cita.hora_inicio }}</span>
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-900 truncate">{{ cita.paciente_nombre }}</p>
                <p class="text-xs text-gray-400">{{ cita.duracion }} min</p>
              </div>
              <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="estadoColor[cita.estado] || 'bg-gray-100 text-gray-600'">
                {{ cita.estado }}
              </span>
            </li>
          </ul>
        ...
        <div class="bg-white shadow-sm rounded-xl overflow-hidden">
          ...
          <ul v-else class="divide-y divide-gray-100">
            <li v-for="s in stats.ultimas_sesiones" :key="s.id" class="px-5 py-3 flex items-center gap-3">
              <div class="flex-1 min-w-0">
